...
public class SellingKMAmount extends Shipment {
    ...
    public BigDecimal getValue(int y, int m, Date d, int type, LinkedHashMap<String, Object> map) {
        ...
        sb.append("select isnull(sum((d.shpamts * h.ratio)/(h.taxrate + 1)),0) from cdrdta d left join cdrhad h on d.shpno=h.shpno LEFT JOIN cdrdmas s on s.cdrno=d.cdrno where ");
        ...
        sb.setLength(0);
        sb.append("select isnull(sum((d.bakamts * h.ratio)/(h.taxrate + 1)),0) from cdrbdta d left join cdrbhad h on d.bakno=h.bakno LEFT JOIN cdrdmas s on s.cdrno=d.cdrno where ");
