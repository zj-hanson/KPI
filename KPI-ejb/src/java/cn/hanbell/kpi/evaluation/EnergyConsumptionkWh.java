 ...
 * @author C1879
 */
public class EnergyConsumptionkWh extends EnergyConsumption {
    ...
    public BigDecimal addItem(int y, int m, Date d, int type, String description, String facno) {
        ...
        sb.append(" SELECT isnull(SUM(KWH),0) FROM ElectricDayReport e INNER JOIN DayReportPara d ON e.TagID=d.ItemName ");
        sb.append(" WHERE  1=1 ");
        ...
        try {
            ...
            kwh = BigDecimal.valueOf(Double.parseDouble(o1.toString()));
        } catch (Exception ex) {
    ...
    public BigDecimal otherItem(int y, int m, Date d, int type, LinkedHashMap<String, Object> map) {
        ...
        BigDecimal kwh = BigDecimal.ZERO;
        StringBuilder sb = new StringBuilder();
        sb.append(" SELECT isnull(SUM(KWH)${condition},0) FROM ElectricDayReport e INNER JOIN DayReportPara d ON e.TagID=d.ItemName ");
