 ...
 * @author C1749 实际值 %的
 */
public class FeedstockPPMPR extends Complaints{
    IndicatorBean indicatorBean = lookupIndicatorBeanBean();

    public FeedstockPPMPR() {
        super();
        queryParams.put("formid", "JL-冷媒进料");
        queryParams.put("deptno", "1M000");
    }

    //Other1(QZ不良数) Other2 （QJ+QC不良数）
    @Override
    public BigDecimal getValue(int y, int m, Date d, int type, LinkedHashMap<String, Object> map) {
        ...
        IndicatorDetail o1 = i.getOther1Indicator();//QZ
        IndicatorDetail o2 = i.getOther2Indicator();//QJ+QC
        IndicatorDetail o3 = i.getOther3Indicator();//总进料
        try {
            ...
            if(a3 != 0){
                ...
                sum = v1.add(v2);//QZ+QJ+QC
                sum = sum.divide(BigDecimal.valueOf(a3), 6,BigDecimal.ROUND_HALF_UP);//除以总进料
                sum = sum.multiply(BigDecimal.valueOf(100));//乘以100等于实际值
                System.out.println(sum);
                return  sum;
            }else{
                System.out.println("总进料为0");
            }
