...
/**
 *
 * @author C1749 QJ 只针对机组
 */
public class FeedstockBadShipmentQJ extends FeedstockBad {
    ...
    public BigDecimal getValue(int y, int m, Date d, int type, LinkedHashMap<String, Object> map) {
        ...
        for (int i = 0; i < mqcuList.size(); i++) {
            if (mqcuList.isEmpty()) {
                ...
                if (LINE.contains("机组")) {//特殊处理 嘉善二加一、缘牵 不考虑不合格单的判定结果
                    if (mqcuList.get(i).getSUPPLYID().equals("SSH00003")) {
                        sb.append(" AND F.RESPONSIBILITYCOMPANY LIKE '%").append(mqcuList.get(i).getSUPPLYNAME()).append("%' ");//厂商简称
                        sb.append(" AND  SUBSTRING(PROJECTID,1,2) in ('QJ') ");
                        sb.append(" AND (");
