...
public class SalesOrderAmount extends SalesOrder {
    ...
    public BigDecimal getNotDelivery(Date d, LinkedHashMap<String, Object> map) {
        ...
        StringBuilder sb = new StringBuilder();
        sb.append(
            "select isnull(sum((d.cdrqy1 - d.shpqy1) * d.unpris * c.ratio/(case c.tax when '4' then (c.taxrate + 1) else 1 end)),0) ");
        sb.append(
            " from cdrhmas c,cdrdmas d,invmas s where c.facno=d.facno and c.cdrno=d.cdrno and c.hrecsta not in ('N','W') ");
        sb.append(" and d.itnbr=s.itnbr ");
        sb.append(" and ((d.cdrqy1-d.shpqy1)>0 or (d.cdrqy2-d.shpqy2)>0) and d.drecsta<'95' and c.facno='${facno}' ");
        if (!"".equals(cusno)) {
            sb.append(" and c.cusno ").append(cusno);
        }
        if (!"".equals(protype)) {
            sb.append(" and left(s.spdsc,2) ").append(protype);
        }
