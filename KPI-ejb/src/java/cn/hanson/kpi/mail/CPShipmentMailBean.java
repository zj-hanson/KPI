...
public class CPShipmentMailBean extends ShipmentMail {
        ...
        try {
            ...
            if (indicator.getActualInterface() != null && indicator.getActualEJB() != null && indicator.getId() != -1) {
                ...
                if (salesOrder != null) {
                    ...
                    num2 = salesOrder.getNotDelivery(d, actualInterface.getQueryParams()).divide(indicator.getRate(), 2,
            ...
            sb.append("<td style=${style}>")
                    .append(decimalFormat.format(indicator.getId() != -1 ? num1 : getData().get("sum1")))
                    .append("</td>");
            // 当月
            // 实际
            ...
            // 改成按天折算
            sb.append("<td style=${style}>")
                    .append(decimalFormat.format(indicatorBean
                            .getValueOfDays(BigDecimal.valueOf(Double.valueOf(f.get(b).toString())), d, 0)))
                    .append("</td>");
            // 成长
            // sb.append("<td
            // style=${style}>").append(percentFormat(indicatorBean.getGrowth(a, b,
            // m))).append("</td>");
            // 改成按天折算
            ...
            // 目标
            // sb.append("<td
            // style=${style}>").append(decimalFormat.format(indicatorBean.getAccumulatedValue(t,
            // m))).append("</td>");
            // 改成按天折算
            ...
            // 达成
            // sb.append("<td
            // style=${style}>").append(percentFormat(indicatorBean.getAccumulatedPerformance(a,
            // t, m))).append("</td>");
            // 改成按天折算
            sb.append("<td style=${style}>")
            ...
            // 同期
            // sb.append("<td
            // style=${style}>").append(decimalFormat.format(indicatorBean.getAccumulatedValue(b,
            // m))).append("</td>");
            // 改成按天折算
            ...
            // 成长
            // sb.append("<td
            // style=${style}>").append(percentFormat(indicatorBean.getAccumulatedGrowth(a,
            // b, m))).append("</td>");
            // 改成按天折算
            ...
            sb.append("<td style=${style}>").append(percentFormat(f.get(p))).append("</td>");
            sb.append("<td style=${style}>")
                    .append(decimalFormat.format(indicator.getId() != -1 ? num2 : getData().get("sum2")))
                    .append("</td>");
            // 年度比重
            if (indicator.getId() == -1 && totalActualValue.compareTo(BigDecimal.ZERO) != 0) {
                sb.append("<td style=${style}>")
                        .append(percentFormat(indicatorBean.getAccumulatedValue(a, m)
    ...
    public String getMailBody() {
        ...
        sb.append(getCustomerTonTable());
        sb.append("<div class=\"tableTitle\">单位：万元</div>");
        sb.append(getCustomerAmtsTable());
    ...
    protected String getCustomerTonTable() {
        ...
        setTotalActualValue("汉声依SHB客户材质出货重量,汉声依THB客户材质出货重量,汉声依OTH客户材质出货重量,汉扬依OTH客户材质出货重量");
        ...
        // HY OTH
        this.indicators.clear();
        this.indicators = indicatorBean.findByCategoryAndYear("汉扬依OTH客户材质出货重量", y);
        indicatorBean.getEntityManager().clear();
        if (indicators != null && !indicators.isEmpty()) {
            for (Indicator i : indicators) {
                indicatorBean.divideByRate(i, 2);
    ...
    protected String getCustomerAmtsTable() {
        sum1 = BigDecimal.ZERO;
        sum2 = BigDecimal.ZERO;
        sumIndicatorList.clear();
        ...
        setTotalActualValue("汉声依SHB客户材质出货金额,汉声依THB客户材质出货金额,汉声依OTH客户材质出货金额,汉扬依OTH客户材质出货金额");
        ...
        // HY OTH
        this.indicators.clear();
        indicators = indicatorBean.findByCategoryAndYear("汉扬依OTH客户材质出货金额", y);
        indicatorBean.getEntityManager().clear();
        if (indicators != null && !indicators.isEmpty()) {
            for (Indicator i : indicators) {
                indicatorBean.divideByRate(i, 2);
    ...
    protected String getVarietyTonTable() {
        ...
        setTotalActualValue("汉声依种类别出货重量,汉扬依种类别出货重量");
        ...
        this.indicators.clear();
        this.indicators = indicatorBean.findByCategoryAndYear("汉扬依种类别出货重量", y);
        indicatorBean.getEntityManager().clear();
        if (indicators != null && !indicators.isEmpty()) {
            for (Indicator i : indicators) {
                indicatorBean.divideByRate(i, 2);
    ...
    protected String getVarietyAmtsTable() {
        ...
        setTotalActualValue("汉声依种类别出货金额,汉扬依种类别出货金额");
        ...
        this.indicators.clear();
        this.indicators = indicatorBean.findByCategoryAndYear("汉扬依种类别出货金额", y);
        indicatorBean.getEntityManager().clear();
        if (indicators != null && !indicators.isEmpty()) {
            for (Indicator i : indicators) {
                indicatorBean.divideByRate(i, 2);
